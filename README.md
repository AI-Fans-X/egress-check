...
很多商家宣称自己的家宽服务是没有做分流的，或者并没有明确的标记，导致用户花了大价钱，以为自己用了家宽，但是最后社交媒体的账号还是被风控了，并且对于为什么被风控毫无头绪。这种情况下，我们需要了解商家是否对某些流量（如 Meta 系、流媒体、金融、电商等平台）做了“线路优化/分流”。

本工具通过批量自动检测近 100 个主流服务，帮助用户快速且准确验证自己的服务器是否存在分流情况。检测原理是用 `mtr` 取每个域名的第一个公网跳，反查其 ASN：
